...
LUI.myGoogleEvent = async function(setup)
{
	try{
		...
		else
		{
			...
			if(setup.unmute)
			{
				LUI.env["vum"] = setup.timeNow.toFixed(2);
				await LUI.events.push({"video_unmute":JSON.stringify(LUI.env)});
			}
			else if(setup.pause)
			{
			...
			{
				if(LUI.env["vum"] > 0 && LUI.env["lvt"] != LUI.env["vt"])
				{
...
LUI.getProductLayer = function(id)
{
	let fid = LUI.getFlowID();
	
	for(var i in LUI.setup["flow"][fid]["flow"])
	{
		console.log(i);
		if(LUI.setup["flow"][fid]["flow"][i] == id)
		{
			return i;
		}
	}
...
window.addEventListener("DOMContentLoaded", async function(){
	...
	if(LUI.set > "")
	{
		...
		elements.forEach(function(item){
			...
			{
				// upsell links
				item.setAttribute("href",LUI.setup.links[item.getAttribute("buylink")][flow_id] + "&" + sparams.toString() );
			}
			else
			{
				// front end links
				item.setAttribute("href",LUI.setup.links[item.getAttribute("buylink")][is_mobile?"mobile":"desktop"] + "&" + sparams.toString() );
			}
		});
	...
	else if(window.location.pathname.match(/\/upsell\-a2\.php|\/upsell\-a3\.php|\/upsell\-a4\.php|\/upsell\-a5\.php|\/thank\-you\.php/))
	{
		if(LUI.env["convs"][LUI.env["conv"]] == false)
		{
			let s = {};
	...
	b_elms.forEach(function(item){
	    ...
	    urlObj.searchParams.set("_b", btoa(LUI.env["hour"] + ";" + LUI.env["page"] + ";" + LUI.env["vtid"] + ";"+ LUI.env["vsl"] + ";" + p["id"] + ";" + p["value"]+ ";" + p["label"]));
		...
		item.addEventListener("mousedown", async function(e){
			...
			if(LUI.env["convs"][ps.id] == undefined)
			{
				...
				if(!LUI.env["buyclick"] && LUI.tracking && LUI.tracking == "lander")
				{
					let s = {};
